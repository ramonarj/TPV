...
struct BulletFighterCollisionMsg: Message {
	BulletFighterCollisionMsg(Uint8 fighterId, Uint32 bulletId) :
			Message(BULLET_FIGHTER_COLLISION, sizeof(BulletFighterCollisionMsg)), fighterId_(
					fighterId), bulletId_(bulletId) {
	}
	Uint8 fighterId_;
	Uint32 bulletId_;
};
...
struct AsteroidCreated : Message {
	AsteroidCreated(Uint32 asteroidId, Vector2D pos, Vector2D dir, Vector2D vel,
		double width, double height) :
		Message(ASTEROID_CREATED, sizeof(AsteroidCreated)), asteroidId_(asteroidId), pos_(pos), dir_(dir), 
		vel_(vel), width_(width), height_(
			height) {
	}
...
struct BulletAsteroidCollisionMsg : Message {
	BulletAsteroidCollisionMsg(Uint32 bulletId) :
		Message(BULLET_ASTEROID_COLLISION, sizeof(BulletFighterCollisionMsg)), bulletId_(bulletId) {
	}
...
struct FighterAsteroidCollisionMsg : Message {
	FighterAsteroidCollisionMsg(Uint32 fighterId) :
		Message(FIGHTER_ASTEROID_COLLISION, sizeof(BulletFighterCollisionMsg)), fighterId_(fighterId) {
	}

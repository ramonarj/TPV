...
SmartGhost::SmartGhost(Texture* texture, Game* juego) : Ghost(texture, juego, 4), age(0), dead(false), childrenTime(0){};
SmartGhost::SmartGhost(Texture* texture, Game* juego, unsigned int posX, unsigned int posY) : Ghost(texture, juego, 4, posX, posY), 
age(5), dead(false), childrenTime(0) { sizeX -= (ADULT_AGE - age); sizeY -= (ADULT_AGE - age); };
...
void SmartGhost::smartMove()
{
	...
	if (dirs.size() > 0)
	{
		Par pacPos; pacPos.x = juego->getPacPos(0); pacPos.y = juego->getPacPos(1);
	...
	while (i < dirs.size() && !flirtSuccess)
	{
		...
		if (juego->hayFantasma(nx, ny, it))
		{
			...
			if (smartGhost != nullptr && smartGhost->isFertile() && this->isFertile() && emptyPlace(babyPos))
			{
				...
				juego->createBabyGhost(babyPos.x, babyPos.y);
			}	
		}
		i++;	
	}
...
bool SmartGhost::emptyPlace(Par& babyPos)
{
	...
	while (i < dirs.size() && !place)
	{
		int nx = posX + dirs[i].x;
		int ny = posY + dirs[i].y;
		if (juego->nextCell(nx, ny) && !juego->hayFantasma(nx, ny, it))
		{
			babyPos.x = nx;
			babyPos.y = ny;
			place = true;
